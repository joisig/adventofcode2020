defmodule Day6 do
  ...
  def parse(lines) do
    Enum.reduce(lines, {[], HashSet.new()}, fn line, {groups, curr} ->
      case line do
        "" ->
          {[curr|groups], HashSet.new()}
        _ ->
          curr = line |> String.graphemes |> Enum.into(curr)
          {groups, curr}
      end
    end)
    |> elem(0)
  ...
  def run1() do
    full_input |> parse |> Enum.map(&(HashSet.size(&1))) |> Enum.sum
  end

  def full_set do
    "abcdefghijklmnopqrstuvwxyz" |> String.graphemes |> Enum.into(HashSet.new())
  end

  def parse2(lines) do
    Enum.reduce(lines, {[], full_set()}, fn line, {groups, curr} ->
      case line do
        "" ->
          {[curr|groups], full_set()}
        _ ->
          curr = line |> String.graphemes |> Enum.into(HashSet.new()) |> HashSet.intersection(curr)
          {groups, curr}
      end
    end)
    |> elem(0)
  ...
  def run2() do
    full_input |> parse2 |> Enum.map(&(HashSet.size(&1))) |> Enum.sum
  end
