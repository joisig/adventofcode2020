defmodule Day2 do
  ...
  def parse(line) do
    [min_s, max_s, char, pwd] = String.split(line, [" ", "-", ":"], trim: true)
    {String.to_integer(min_s), String.to_integer(max_s), char, String.graphemes(pwd)}
  end

  def check1({min, max, char, graphemes}) do
    num = graphemes |> Enum.filter(&(&1 == char)) |> Kernel.length
    num >= min && num <= max
  end

  def run1() do
    #for line <- get_lines() |> Enum.slice(0, 10), do: line |> parse |> check
    (for line <- get_lines(), do: line |> parse |> check1) |> Enum.filter(&(&1 == true)) |> Kernel.length
  end

  def check2({i1, i2, char, graphemes}) do
    key_chars = [Enum.fetch!(graphemes, i1 - 1), Enum.fetch!(graphemes, i2 - 1)]
    (key_chars |> Enum.filter(&(&1 == char)) |> Kernel.length) == 1
  end

  def run2() do
    #for line <- get_lines() |> Enum.slice(0, 10), do: line |> parse |> check
    (for line <- get_lines(), do: line |> parse |> check2) |> Enum.filter(&(&1 == true)) |> Kernel.length
  end
