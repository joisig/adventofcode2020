defmodule Day7 do
  ...
  def parse_line(line, opts) do
    case Regex.run(~r/(.+) bags contain (.+)/, line) do
      [_, outer, "no other bags."] ->
        {outer, []}
      [_, outer, inners] ->
        inners = String.split(inners, ", ")
        |> Enum.map(fn i ->
          [_, num, color] = Regex.run(~r/(\d+) (.+) bags?\.?/, i)
          case Keyword.get(opts, :include_number, false) do
            true ->
  ...
  def parse(lines, opts \\ []) do
    Enum.map(lines, &(parse_line(&1, opts)))
  end

  def invert(rules) do
    Enum.reduce(rules, %{}, fn {outer, inners}, acc ->
      acc = Map.put_new(acc, outer, [])
      Enum.reduce(inners, acc, fn inner, acc2 ->
        case Map.fetch(acc2, inner) do
          :error ->
            Map.put(acc2, inner, [outer])
          {:ok, list} ->
            Map.put(acc2, inner, [outer|list])
        end
      end)
    end)
  end

  def reachable(inverted, color) do
    direct_reachable = Map.get(inverted, color)
    Enum.reduce(direct_reachable, direct_reachable, fn reachable_color, acc ->
      reachable(inverted, reachable_color) ++ acc
    end)
    |> Enum.uniq
  end

  def numrules(lines) do
    parse(lines, include_number: true) |> Enum.into(%{})
  end

  def count_bags(numrules, color) do
    contained = Map.get(numrules, color)
    Enum.reduce(contained, 0, fn {contained_color, num}, acc ->
      acc = acc + num + num * count_bags(numrules, contained_color)
    end)
  end
