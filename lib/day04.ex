defmodule Day4 do
  ...
  def parse_input(str) do
    str
    |> String.split("\n")
    |> Enum.reduce({[], %{}}, fn line, {items, curr} = acc ->
      case line do
        "" ->
          {[curr|items], %{}}
        _ ->
          curr = parse_line(line) |> Enum.into(curr)
          {items, curr}
      end
    end)
  end

  def validate1(items) do
    items |> Enum.filter(fn item ->
      case item do
        %{"byr" => byr, "iyr" => iyr, "eyr" => eyr, "hgt" => hgt, "hcl" => hcl, "ecl" => ecl, "pid" => pid} = item ->
          true
        _ ->
          false
      end
  ...
  def validate_year(year, min_year, max_year) do
    case Regex.run(~r/^[0-9]{4}$/, year) do
      [match] ->
        num = String.to_integer(match)
  ...
  def validate_byr(byr) do
    validate_year(byr, 1920, 2002)
  end

  def validate_iyr(iyr) do
    validate_year(iyr, 2010, 2020)
  end

  def validate_eyr(eyr) do
    validate_year(eyr, 2020, 2030)
  end

  def validate_hgt(hgt) do
    case Regex.run(~r/^([0-9]+)(cm|in)$/, hgt) do
      [_, num_str, type] ->
  ...
  def validate_hcl(hcl) do
    Regex.match?(~r/^(#[0-9a-f]{6})$/, hcl)
  end

  def validate_ecl(ecl) do
  ...
  def run1() do
    {items, _} = full_input()
    items |> validate1 |> length
  end

  def run2() do
    {items, _} = full_input()
    items |> validate1 |> validate2 |> length
  end
